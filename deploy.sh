...
ERR=$?
$S_LOG -s $ERR -d $S_NAME "APT INSTALL of zabbix-agent returned code $ERR"
[ "$ERR" = "0"  ] || exit 1
...
sed -i -r "s/^Include=\/etc\/zabbix\/zabbix_agentd.d\/$/Include=\/etc\/zabbix\/zabbix_agentd.d\/*.conf/g" $ZBX_CONF
$S_LOG -s $? -d $S_NAME "Edited for old Zabbix version: $(egrep ^Include= $ZBX_CONF)"
